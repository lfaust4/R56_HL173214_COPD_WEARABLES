...
#####auc graph
layout(matrix(c(1, 2), nrow = 2), heights = c(4, 1))  # 4:1 height ratio
par(mar = c(4, 4, 4, 2))  # normal margins
plot(roc_pls, col = "blue", legacy.axes = TRUE,
     main = "",
     print.auc = TRUE, auc.polygon = TRUE, auc.polygon.col = rgb(0, 0, 1, 0.1))
plot(roc_no_pls, col = "red", add = TRUE, print.auc = TRUE,
     print.auc.y = 0.4, auc.polygon = TRUE, auc.polygon.col = rgb(1, 0, 0, 0.1))


par(mar = c(0, 0, 0, 0))  # no margins
plot.new()
legend("center",
       legend = c(paste("AUC (Decision Tree with Composite Sleep Health Score) =", round(auc1, 3)),
                  paste("AUC (Decision Tree without Composite Sleep Health Score) =", round(auc2, 3))),
       col = c("blue", "red"), lwd = 2, horiz = FALSE, bty = "n", cex = 1.1)
...
# Extract the underlying fitted tree model
tree_model <- extract_fit_parsnip(fit_tree)$fit
# For rpart models, variable importance is stored here:
...
plot.new()
legend("center",
       legend = c(paste("AUC (Logistic Regression with Composite Sleep Health Score) =", round(auc1, 3)),
                  paste("AUC (Logistic Regression without Composite Sleep Health Score) =", round(auc2, 3))),
       col = c("blue", "red"), lwd = 2, horiz = FALSE, bty = "n", cex = 1.1)
...
fit_tree1 <- fit(wf_no_pls, data = df_imputed)
tree_model1 <- extract_fit_parsnip(fit_tree1)$fit
vip(tree_model1)  # plots importance

library(vip)
importance_df <- vi(tree_model1)
importance_df

fit_tree2 <- fit(wf_pls, data = df_imputed)
tree_model2 <- extract_fit_parsnip(fit_tree2)$fit
vip(tree_model2)  # plots importance

importance_df <- vi(tree_model2)
importance_df
...
auc1 <- auc(roc_pls_svm)
auc2 <- auc(roc_no_pls_svm)
print(auc1)
print(auc2)
...
plot.new()
legend("center",
       legend = c(paste("AUC (SVM with Composite Sleep Health Score) =", round(auc1, 3)),
                  paste("AUC (SVM without Composite Sleep Health Score) =", round(auc2, 3))),
       col = c("blue", "red"), lwd = 2, horiz = FALSE, bty = "n", cex = 1.1)
...
final_fit <- fit(wf_pls_svm, data = df_imputed)
svm_fit <- extract_fit_parsnip(final_fit)
importance2 <- abs(svm_fit$fit$W)   # numeric vector
importance2

# Drop the bias column
importance_no_bias <- importance2[, colnames(importance2) != "Bias", drop = FALSE]
...
svm_fit <- extract_fit_parsnip(final_fit)
importance3 <- abs(svm_fit$fit$W)   # numeric vector
importance3

# Drop the bias column
importance_no_bias <- importance3[, colnames(importance3) != "Bias", drop = FALSE]
...
# Delong's test
roc.test(roc_pls_nb, roc_no_pls_nb, method = "delong")

# Default is 95% CI
ci_pls <- ci.auc(roc_pls_nb)
ci_no_pls <- ci.auc(roc_no_pls_nb)
...
layout(matrix(c(1, 2), nrow = 2), heights = c(4, 1))  # 4:1 height ratio
par(mar = c(4, 4, 4, 2))  # normal margins
plot(roc_pls_nb, col = "blue", legacy.axes = TRUE,
     main = "",
     print.auc = TRUE, auc.polygon = TRUE, auc.polygon.col = rgb(0, 0, 1, 0.1))
plot(roc_no_pls_nb, col = "red", add = TRUE, print.auc = TRUE,
     print.auc.y = 0.4, auc.polygon = TRUE, auc.polygon.col = rgb(1, 0, 0, 0.1))
...
plot.new()
legend("center",
       legend = c(paste("AUC (Naive Bayes with Composite Sleep Health Score) =", round(auc1, 3)),
                  paste("AUC (Naive Bayes without Composite Sleep Health Score) =", round(auc2, 3))),
       col = c("blue", "red"), lwd = 2, horiz = FALSE, bty = "n", cex = 1.1)
